...
public class Surface
{
    ...
    {
        // minimum subdivsion of one
        int vertCount = 0;
        for (int i = 0; i < controlPatch.pointData.Length; i++)
        {
            vertCount += controlPatch.pointData[i].Length;
        }
        // for the corners of triangle
        vertCount -= 3;

        int triangleCount = 0;
        for (int i = 1; i < controlPatch.pointData.Length - 1; i++)
        {
            triangleCount += (controlPatch.pointData[i].Length - 1) + (controlPatch.pointData[i].Length - 2);
        }
        triangleCount -= (controlPatch.pointData[controlPatch.pointData.Length - 2].Length - 2);
        triangleCount *= 3;
        ...
        int triangleIndex = 0;
        int vertIndex = 0;
        for (int i = 1; i < controlPatch.pointData.Length; i++)
        {
            for (int j = 0; j < controlPatch.pointData[i].Length; j++)
            {
                if (i == controlPatch.pointData.Length - 1 && (j == 0 || j == controlPatch.pointData[i].Length - 1)) continue;

                if (j > 0 && i < controlPatch.pointData.Length - 1)
                {
                    int pointCountTillNextRow = controlPatch.pointData[i].Length - (j + 1);
                    if (i == controlPatch.pointData.Length - 2) pointCountTillNextRow--;
                    triangles[triangleIndex + 0] = vertIndex;
                    triangles[triangleIndex + 1] = vertIndex + pointCountTillNextRow + (j + 1);
                    triangles[triangleIndex + 2] = vertIndex - 1;
                    triangleIndex += 3;
                    if (j < controlPatch.pointData[i].Length - 1 && i < controlPatch.pointData.Length - 2)
                    {
                        triangles[triangleIndex + 0] = vertIndex;
                        triangles[triangleIndex + 1] = vertIndex + pointCountTillNextRow + (j + 2);
                        triangles[triangleIndex + 2] = vertIndex + pointCountTillNextRow + (j + 1);
                        triangleIndex += 3;
                    }
                }
